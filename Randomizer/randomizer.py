...
cfgInDir = '/usr/share/emulationstation/es_systems.cfg'
# TODO verify this
romsInDir = '/userdata/roms'
# TODO verify this
logDir= '/userdata/'
# TODO verify what fake system to add there
...
# TODO Fix display size of image
# def showPic(image,syst) :
    ...
    # if os.path.exists(file) :
        ...
        #TODO COMPUTE SMALLER DISPLAY PORT ON SCREEN AND REDUCE IMAGE TO IT
        ...
        #TODO might one day listen to button event to allow changing random selected game
        # time.sleep(5)
...
def selectGame(system,games,ctrlStr):    
    ...
    log('')
    log("out: "+command)
    # TODO verify command line, check through ssh what it looks like before
    os.popen(command)
...
def init() :
    ...
    if respo.lower() not in ['y','yes'] :
        ...
        sys.exit()
    randomDir = os.path.join(romsInDir, 'random')
    allGenres = dict()
    arcadeGenres = dict()
...
if __name__ == "__main__":
    ...
    log('')
    # Load systems from es_systems.cfg (share_init) which have a gamelist
    ...
    if sys.argv[-1].lower() == 'init' or len(sys.argv) == 1 :
        ...
        rdmParams = readRdmFile(sys.argv[-1])
        ...
        while(len(paramSystems) > 0) :
            ...
            if possibleGames is not None and len(possibleGames) > 0 :
                selectGame(launchSyst,possibleGames,ctrlStr)
                break;
            else :
